...
for i=1:size(X,2)
   ...
   mwModel.PLSmodel{i}=sub_pls_val(X(:,selectedwindow),Y,no_of_lv,prepro_method,val_method,mwModel.segments);
end

mwModel.PLSmodel{size(X,2)+1}=sub_pls_val(X,Y,no_of_lv,prepro_method,val_method,mwModel.segments);
